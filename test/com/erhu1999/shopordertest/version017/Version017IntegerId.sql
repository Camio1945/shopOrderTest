...
CREATE TABLE `Goods`  (
  ...
  `price` int(10) NOT NULL DEFAULT 0 COMMENT '销售价（单位为分）',
...
CREATE TABLE `Order`  (
  ...
  `money` int(10) NOT NULL COMMENT '订单金额（单位为分）',
...
CREATE TABLE `OrderGoods`  (
  ...
  `goodsPrice` int(10) NOT NULL COMMENT '商品单价（单位为分）',
  `goodsImg` varchar(100) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL COMMENT '商品图片',
  `money` int(10) NOT NULL COMMENT '商品金额（单位为分）',
  PRIMARY KEY (`id`) USING BTREE
