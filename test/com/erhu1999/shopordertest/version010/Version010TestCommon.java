...
import static com.erhu1999.shopordertest.version010.JdbcUtil010.dropDbIfExistsThenCreateDb;
import static com.erhu1999.shopordertest.version010.JdbcUtil010.execSqlFile;
import static com.erhu1999.shopordertest.version010.JdbcUtil010.init;
import static com.erhu1999.shopordertest.version010.JdbcUtil010.queryOneRow;
import static com.erhu1999.shopordertest.version010.JdbcUtil010.renewUrl;
import static org.junit.jupiter.api.Assertions.assertEquals;

/**
 * 版本010的测试用例
 */
class Version010TestCommon extends AbstractTest {

    /** 数据库名称 */
    private static final String DB_NAME = DB_NAME_PREFIX + "version010";
    /** 数据库连接数量 */
    ...
    static void initDb() throws SQLException {
        printSeparateLine(Version010TestCommon.class.getSimpleName(), new Exception().getStackTrace()[0].getMethodName());
        ...
        // 重新初始化数据库连接相关的参数
        renewUrl(DB_NAME);
        String sqlFilePath = Version010TestCommon.class.getResource("").getFile() + Version010TestCommon.class.getSimpleName() + ".sql";
    ...
    private static void initDataSourceByHikari() throws SQLException {
        ...
        // 切换数据源
        JdbcUtil010.setDataSource(hikariDataSource);
     ...
     * @param isMoreSynchronize 是否同步（的代码块）多一些，如果是，则调用Version010Method1类，否则调用Version010Method2类
     * @throws Exception
     */
    static void testMultiThread(int threadCount, boolean isMoreSynchronize) throws Exception {
        // 商品ID
        Long goodsId = 1L;
        // 查询商品信息
        ...
        Version010Method1 version010Method1 = new Version010Method1();
        Version010Method2 version010Method2 = new Version010Method2();
        for (int threadIndex = 0; threadIndex < threadCount; threadIndex++) {
            ...
            new Thread(() -> {
                ...
                for (int i = 0; i < times; i++) {
                    // 调用提交订单的接口
                    try {
                        if (isMoreSynchronize) {
                            version010Method1.submitOrder(userId, goodsId, goodsCount, addrId);
                        } else {
                            version010Method2.submitOrder(userId, goodsId, goodsCount, addrId);
                        }
                    } catch (Exception e) {
