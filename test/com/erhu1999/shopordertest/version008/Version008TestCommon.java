...
import static com.erhu1999.shopordertest.version008.JdbcUtil008.dropDbIfExistsThenCreateDb;
import static com.erhu1999.shopordertest.version008.JdbcUtil008.execSqlFile;
import static com.erhu1999.shopordertest.version008.JdbcUtil008.init;
import static com.erhu1999.shopordertest.version008.JdbcUtil008.queryOneRow;
import static com.erhu1999.shopordertest.version008.JdbcUtil008.renewUrl;
import static org.junit.jupiter.api.Assertions.assertEquals;

/**
 * 版本008的测试用例
 */
class Version008TestCommon extends AbstractTest {

    /** 数据库名称 */
    private static final String DB_NAME = DB_NAME_PREFIX + "version008";
    /** 数据库连接数量 */
    ...
    static void initDb() throws SQLException {
        printSeparateLine(Version008TestCommon.class.getSimpleName(), new Exception().getStackTrace()[0].getMethodName());
        ...
        // 重新初始化数据库连接相关的参数
        renewUrl(DB_NAME);
        String sqlFilePath = Version008TestCommon.class.getResource("").getFile() + Version008TestCommon.class.getSimpleName() + ".sql";
    ...
    private static void initDataSourceByHikari() throws SQLException {
        ...
        // 切换数据源
        JdbcUtil008.setDataSource(hikariDataSource);
     ...
     * @throws Exception
     */
    static void testMultiThread(int threadCount, boolean isStatic) throws Exception {
        // 商品ID
        Long goodsId = 1L;
        // 查询商品信息
        ...
        Version008Normal version008 = new Version008Normal();
        for (int threadIndex = 0; threadIndex < threadCount; threadIndex++) {
            ...
            new Thread(() -> {
                ...
                for (int i = 0; i < times; i++) {
                    ...
                    try {
                        if (isStatic) {
                            Version008Static.submitOrderSynchronized(userId, goodsId, goodsCount, addrId);
                        } else {
                            version008.submitOrderSynchronized(userId, goodsId, goodsCount, addrId);
                        }
                    } catch (Exception e) {
